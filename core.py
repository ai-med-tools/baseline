...
sio.register_namespace(BaselineNamespace('/baselinejxowumdzzbap'))
...
def native_baseline_queue():
    ...
    while True:
        ...
        if msg["op"] == "start":
            ...
            sio.emit('session-start', msg["data"], "/baselinejxowumdzzbap")
        ...
        if msg["op"] == 'gettest':
            try:
                ...
                sio.emit("session-get-test", msg["data"], "/baselinejxowumdzzbap")
...
if __name__ == "__main__":
    ...
    with daemon.DaemonContext():
        ...
        sio.connect(f'{perfomance["aimed_host"]}?token={perfomance["token"]}', namespaces=['/baselinejxowumdzzbap'], transports=['websocket'])
